...
class Tracks:
    ...
    def get_by_id(id_songs):
        db = get_db()
        cursor = db.cursor()
        cursor.execute("SELECT * FROM songs WHERE id_songs = ?", (id_songs,))
        row = cursor.fetchone()
        cursor.close()
        if row:
            return Tracks(id_songs=row["id_songs"], tittle=row["tittle"], duracion=row["duracion"], fecha_subida=row["fecha_subida"])
        return None
    ...
    def get_all():
        ...
        canciones = [Tracks(id_songs=row["id_songs"], tittle=row["tittle"], duracion=row["duracion"], fecha_subida=row["fecha_subida"]) for row in rows]
        cursor.close()
        return canciones
    ...
    def save(self):
        ...
        if self.id_songs:
            cursor.execute("""
                UPDATE songs SET tittle = ?, duracion = ?, fecha_subida = ?
                WHERE id_songs = ?
            """, (self.tittle, self.duracion, self.fecha_subida, self.id_songs))
        else:
            cursor.execute("""
                INSERT INTO songs (tittle, duracion, fecha_subida)
                VALUES (?, ?, ?)
    ...
    def delete(self):
        db = get_db()
        cursor = db.cursor()
        cursor.execute("DELETE FROM songs WHERE id_songs = ?", (self.id_songs,))
        db.commit()
        cursor.close()
